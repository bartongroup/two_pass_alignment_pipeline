...
def get_bed12_linked_splicing(bed_fn):
    with open(bed_fn) as bed:
        for record in bed:
            ...
            strand = record[5]
            read_id = record[3]
            start, end, exons = parse_exons(record)
            introns = exons_to_introns(exons)
            linked_splices = set()
            for inv in introns:
                linked_splices.add(f'{chrom}:{inv[0]}-{inv[1]}({strand})')
            yield read_id, linked_splices
...
def get_gtf_linked_splicing(gtf_fn):
    ...
    for transcript_id, chrom, strand, exons in parse_gtf_exon_invs(gtf_fn):
        introns = exons_to_introns(exons)
        linked_splices[transcript_id] = set()
        for inv in introns:
...
def score_sim_alignment(aln_bed_fn, reference_splices, remove_version=False):
    ...
    res = {}
    quant_res = Counter()
    for read_id, splice in get_bed12_linked_splicing(aln_bed_fn):
        ...
        try:
            ...
            warnings.warn(f'Simulated transcript {transcript_of_origin} not in GTF')
            continue
        if not len(splice) and not len(correct_splicing):
            # read is correctly unspliced
            res[read_id] = 'unspliced_correctly'
            quant_res[transcript_of_origin] += 1
        elif not len(splice) and len(correct_splicing):
            res[read_id] = 'unspliced_incorrectly' # read is incorrectly unspliced
        else:
            # both read and reference are spliced
            if splice == correct_splicing:
                res[read_id] = 'spliced_correctly' # splice is correct
                quant_res[transcript_of_origin] += 1
            elif splice in ref_splice_inv:
                res[read_id] = 'spliced_incorrectly_annot' # read maps to incorrect transcript
                quant_res[ref_splice_inv[splice]] += 1
            else:
                res[read_id] = 'spliced_incorrectly_unannot' # read is incorrectly spliced
...
def main(bed12_fn, annot_gtf_fn, output_fn, output_quant_fn, remove_ensembl_transcript_version):
    ...
    res, quant_res = score_sim_alignment(
        bed12_fn, reference_splices,
        remove_ensembl_transcript_version
    )
